...
export function useGlobalSearch(workspaceId: string, query: string) {
  ...
  const results = useMemo(() => {
    if (!query || query.trim() === '') return { projects: [], tasks: [], notes: [], resources: [], members: [], habits: [] };
    ...
    const allMembers = extractItems<{ id: string; email?: string; role: string; user?: { displayName?: string } }>(['members', workspaceId]);
    ...
    const tasks: SearchResult[] = allTasks
      ...
      .map(t => ({
        ...
        path: t.projectId ? `/w/${workspaceId}/projects/${t.projectId}/tasks/${t.id}` : `/w/${workspaceId}/tasks/${t.id}`,
    ...
    const habits: SearchResult[] = allHabits
      ...
      .map(h => ({
        ...
        path: h.projectId ? `/w/${workspaceId}/projects/${h.projectId}/habits` : `/w/${workspaceId}/habits`,
    ...
    const notes: SearchResult[] = allNotes
      ...
      .map(n => ({
        ...
        type: 'note',
        title: n.title,
        subtitle: n.content ? (n.content.length > 50 ? n.content.substring(0, 50) + '...' : n.content) : undefined,
    ...
    const resources: SearchResult[] = allResources
      ...
      .map(r => ({
        ...
        path: r.projectId ? `/w/${workspaceId}/projects/${r.projectId}/resources` : `/w/${workspaceId}/dashboard`,
