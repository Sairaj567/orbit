...
> **Status: Functional but Missing Polish.** CRUD, Markdown edit/preview, pinning, project/task fields, local search, activity, and embedding requests exist. Server search, author/history, robust entity validation, and matching realtime invalidation are incomplete. The current schema has no `authorId` field.
